...
public class S3Service {
    ...
    private final S3Client s3Client;        // IAM이 생성돼 있는 객체
    ...
    public String upload(MultipartFile multipartFile) throws IOException {
        ...
        String dirName = "images"; //Key - Value 형태로 저장되지만, Key의 슬래시를 이용하여 폴더 개념 같이 사용 가능
                                    // /images/cloud.png 와 같은 url 로 구성된다.
        String fileName = dirName + "/" + randomName; //만들어진 dirName 폴더 뒤에 ramdomName 을 붙인다.

        PutObjectRequest putObjectRequest = PutObjectRequest.builder() //PutObjectRequest는 AWS SDK v2에서 제공하는 객체로
                                                            // S3에 파일을 업로드할 때 필요한 정보를 포함하는 요청(Request) 객체이다.
                .bucket(bucket)
                .key(fileName)
                .contentType(multipartFile.getContentType())
                .build();

        // 파일 바이트 배열로 바꾼 후 s3에 업로드
        s3Client.putObject(putObjectRequest, RequestBody.fromBytes(multipartFile.getBytes()));

        // s3에서 주소를 받아오기 위해 정보 담은 GetUrlRequest 객체 만들기
